...
DEVELOPER_USERNAME = os.getenv("DEVELOPER_USERNAME", "developer").strip().lower()
...
class ContactDeveloperIn(BaseModel):
...
async def fetch_delivery_statuses(
    conn: asyncpg.Connection,
    message_ids: list[str],
    current_user_id: str,
) -> dict[str, str]:
    ...
    rows = await conn.fetch(
        ...
        SELECT
            message_id,
            BOOL_OR(read_at IS NOT NULL) AS has_read,
            BOOL_OR(delivered_at IS NOT NULL OR read_at IS NOT NULL) AS has_delivered
    ...
    for row in rows:
        if row["has_read"]:
            statuses[row["message_id"]] = "read"
        elif row["has_delivered"]:
            statuses[row["message_id"]] = "delivered"
...
async def fetch_message_payload(
    ...
    delivery_statuses = await fetch_delivery_statuses(
        conn,
        [message_id],
        current_user_id,
    )
    ...
    return {
        ...
        "delivery_status": delivery_statuses.get(row["id"], "sent")
        if sender_id == current_user_id
        else None,
        "forwarded_from_message_id": row["forwarded_from_message_id"],
        "forwarded_from_user_id": row["forwarded_from_user_id"],
        "forwarded_from_name": (
            row["forwarded_from_display_name"]
            or row["forwarded_from_nickname"]
            or row["forwarded_from_username"]
        ),
        "pinned": row["pinned_message_id"] == row["id"],
    }


async def broadcast_receipt_updates(
    conn: asyncpg.Connection,
    message_ids: list[str],
    status: str,
) -> None:
    ...
    for row in rows:
        chat_message_ids = await conn.fetch(
            """
            SELECT id
            FROM public.messages
            WHERE chat_id = $1
        ...
        ids = [item["id"] for item in chat_message_ids]
...
async def get_or_create_direct_chat_between(
    conn: asyncpg.Connection,
    user_id: str,
    other_user_id: str,
) -> dict:
...
async def startup():
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS message_receipts_message_user_idx
...
async def mark_messages_delivered(
    ...
    message_ids = [clean_text(item) for item in data.message_ids]
    ...
    async with db_pool.acquire() as conn:
        ...
        await broadcast_receipt_updates(conn, allowed_ids, "delivered")

    return {"ok": True, "message_ids": allowed_ids}
...
async def mark_chat_messages_read(
    ...
    async with db_pool.acquire() as conn:
        ...
        await broadcast_receipt_updates(conn, message_ids, "read")
...
async def get_pinned_message(
    ...
    async with db_pool.acquire() as conn:
        ...
        message = await fetch_message_payload(conn, pinned_message_id, user_id)
...
async def set_pinned_message(
    ...
    async with db_pool.acquire() as conn:
        ...
        message = await fetch_message_payload(conn, message_id, user_id) if message_id else None
...
async def forward_message(
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            await assert_chat_member(conn, source["chat_id"], user_id)
            ...
            if source["attachment_id"]:
                ...
                original_name = source["attachment_original_name"] or "file"
                new_storage_key = f"{user_id}/forwarded/{new_attachment_id}_{safe_filename(original_name)}"
                content = source["attachment_file_bytes"]

                source_path = UPLOAD_DIR / source["attachment_storage_key"]
                target_path = UPLOAD_DIR / new_storage_key
                ...
                new_attachment = {
                    ...
                    "url": attachment_url(new_storage_key),
                    "storage_key": new_storage_key,
                    "original_name": original_name,
                    "mime_type": source["attachment_mime_type"],
                    "size_bytes": source["attachment_size_bytes"],
                    "kind": source["attachment_kind"],
                }
    ...
    forwarded_from_name = source["display_name"] or source["nickname"] or source["username"]
    payload = {
        "id": new_message_id,
        ...
        "forwarded_from_user_id": source["sender_user_id"],
...
async def delete_messages_batch(
    ...
    for chat_id in chat_ids:
        ids_for_chat = [row["id"] for row in rows if row["chat_id"] == chat_id]
        await manager.broadcast_to_chat(
            chat_id,
            {
                "type": "messages.deleted",
                "chat_id": chat_id,
                "message_ids": ids_for_chat,
            },
        )
...
async def open_developer_chat(
    ...
    async with db_pool.acquire() as conn:
        ...
        if not developer and DEVELOPER_USERNAME:
            developer = await conn.fetchrow(
                """
                SELECT id, username, nickname, display_name
                FROM public.users
        ...
        async with conn.transaction():
            chat = await get_or_create_direct_chat_between(
                conn,
                user_id,
                developer["id"],
            )
            ...
            if message_text:
                ...
                sent_message = {
                    ...
                    "sender_name": sender["display_name"] or sender["nickname"] or sender["username"],
                    "text": message_text,
                    "message_type": "text",
                    "attachment": None,
                    "reactions": [],
    ...
    if chat.get("already_exists") is False:
        await manager.send_to_user(
            developer["id"],
            {
                "type": "chat.created",
    ...
    return {"ok": True, "chat": chat, "message": sent_message}
